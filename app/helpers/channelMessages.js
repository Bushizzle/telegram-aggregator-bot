...
const getDistrict = (message, districts) => districts.find((d) => d.values.some((val) => message.toLowerCase().match(REGEXP.make.district(val.toLowerCase()))));

const getConfig = (id, configs) => configs.find((conf) => conf.id === id);

const isDistrict = (str, districts) => districts.find((el) => el.values.some((val) => str.trim().toLowerCase() === val.trim().toLowerCase()));
