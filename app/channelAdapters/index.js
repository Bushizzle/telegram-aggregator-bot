...
const adapterByKeys = (message, config) => (config?.keys?.length ? mapStrings(message).reduce((res, str) => {
  const keyPair = getKeypair(config, str);
  if (keyPair) res[keyPair.key] = removeGarbage(getValue(str, keyPair.value), keyPair.key);
  return res;
}, {}) : {});

const adapterByMarkers = (message, config) => (config?.markers?.length ? mapStrings(message).reduce((res, str) => {
  const marker = config.markers.find(({ matches, exceptions }) => matches.some((val) => str.match(val) && (!exceptions || !haveExceptions(str, exceptions))));
